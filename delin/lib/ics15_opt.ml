(* Performance-tuned variant of the reference [Ics15] driver. Same
   ...
   candidate atoms and so is permutation-invariant:
   ...
   (C) Derive alphas from a per-atom table. In the reference,
   ...
   depends only on which atom is dropped, not on its position. So we
   build a table [quot a] once, with one entry per candidate atom, and
   ...
   survives alpha-derivation exactly when every atom after the first
   has a defined quotient. Two consequences for pruning:
   - two or more atoms with an undefined quotient: no permutation can
     place both at position 0, so the search returns nothing;
   - otherwise we keep only the permutations whose tail avoids the
     undefined-quotient atom, skipping the expensive subscript-recovery
...
(* [lookup a] is the precomputed quotient table; the [survives] filter
   guarantees every tail atom has [Some], so [Option.get] is safe. *)
let try_permutation
    ~(buckets : Expr.t Term_inner.Map.t)
    ~(f0 : Expr.t)
    ~(lookup : Atom.t -> int option)
    ~(expr : Expr.t)
    (perm : Atom.t list)
    : Index.t option =
  ...
  if Expr.compare (Index.reconstruct idx) expr = 0 then Some idx else None

let candidates ~globals:_ ~size_params expr =
  let atoms = params_in_size_params size_params in
  let buckets = Polynomial.group_by_parameters ~candidates:atoms expr in
  let f0 = bucket_lookup buckets atoms in
  let d = List.length atoms + 1 in
  ...
  if d > 1 && Expr.compare f0 Expr.zero = 0 then Seq.empty
  else
    (* (C) one quotient per candidate atom, keyed on "drop this atom". *)
    let quot_tbl =
      List.fold_left
        (fun m a ->
          let others =
            List.filter (fun b -> Atom.compare a b <> 0) atoms
          in
          let q =
            Polynomial.try_scalar_quotient (bucket_lookup buckets others) f0
          in
          Atom.Map.add a q m)
        Atom.Map.empty atoms
    in
    let lookup a = Atom.Map.find a quot_tbl in
    let bad_count =
      List.fold_left
