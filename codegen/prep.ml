...
let prepare_params (g : Generator.t) (gv : Gv_parser.t) : Gv_parser.t * Config.t
    =
...
let rec inst_to_vars : Code.t -> Variable.t list = function
  | Skip -> []
  | Decl { body = p; _ } -> inst_to_vars p
  | Access e -> List.map n_to_vars e.index |> List.flatten
  | Sync _ -> []
...
and r_to_vars (r : Range.t) : Variable.t list =
  let step_variables =
    match r.step with Plus e -> n_to_vars e | Mult e -> n_to_vars e
  in
  [
...
let variables_used (p : Code.t) : VarSet.t = inst_to_vars p |> VarSet.of_list
...
let prepare_kernel (g : Generator.t) (params : Config.t) (k : Kernel.t) :
    Kernel.t =
  k
  |> (if g.const_fold then Kernel.opt else Fun.id)
  |> remove_unused_variables |> mk_types_compatible
