...
module Code = struct
  ...
  let rec from_proto : Protocols.Code.t -> t Seq.t =
    ...
    | If (b, p, q) ->
      Seq.append
        ...
        (from_proto q |> Seq.map (fun q -> Cond {test=Exp.b_not b; body=q}))
  ...
  let rec is_uniform (thread_locals:Variable.Set.t) : t -> bool =
    ...
    | Cond {test=b; body=s} -> b_is_uniform thread_locals b && is_uniform thread_locals s

  let rec location : t -> Location.t option =
    function
    | Barrier l -> l
    | Decl {body=s; _} | Cond {body=s; _} | Loop {body=s; _} -> location s
