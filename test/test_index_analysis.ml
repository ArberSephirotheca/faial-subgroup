...
let assert_bc ?(cfg = cfg) ?(locals = Variable.Set.empty) ~expected ~given () :
    unit =
  let given = Index_analysis.BC.from_nexp cfg locals given in
  let msg =
    "Expected: "
    ^ Index_analysis.BC.to_string expected
    ^ "\n" ^ "Given: "
    ^ Index_analysis.BC.to_string given
  in
  assert_equal expected given ~msg

let assert_ua ?(cfg = cfg) ?(locals = Variable.Set.empty) ~expected
    ~(given : Exp.nexp) () : unit =
  let given = Index_analysis.UA.from_nexp cfg locals given in
  let msg =
    "Expected: "
    ^ Index_analysis.UA.to_string expected
    ^ "\n" ^ "Given: "
    ^ Index_analysis.UA.to_string given
  in
  assert_equal expected given ~msg
