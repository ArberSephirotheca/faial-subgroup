...
let scoped_testable : Scoped.t Alcotest.testable =
  let pp fmt s = Format.fprintf fmt "%s" (Scoped.to_string s) in
  let equal = ( = ) in
  Alcotest.testable pp equal

(* Test helper functions *)
let test_scoped_conversion (name : string) (stmt : Stmt.t) (expected : Scoped.t) =
  ( name,
    `Quick,
    fun () ->
      let _, actual = Scoped.from_stmt (Params.empty, stmt) in
