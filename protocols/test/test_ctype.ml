...
let test_split_array_type () : unit =
  ...
  List.iter (fun (given, expected) ->
    Alcotest.check Alcotest.(option (pair string string)) ("split_array_type " ^ given) expected (split_array_type given)
  ) test_cases

let test_parse_dim () : unit =
  Alcotest.check Alcotest.(list int) "parse_dim [8][8]" [8; 8] (parse_dim "[8][8]");
  Alcotest.check Alcotest.(list int) "parse_dim [128]" [128] (parse_dim "[128]")
...
let test_parse_array_type_opt () : unit =
  ...
  List.iter (fun (given, expected) ->
    Alcotest.check Alcotest.(option (list string)) ("parse_array_type_opt " ^ given) expected (parse_array_type_opt given)
  ) test_cases
...
let test_parse_array_dim_opt () : unit =
  ...
  List.iter (fun (given, expected) ->
    Alcotest.check Alcotest.(option (list int)) ("parse_array_dim_opt " ^ given) expected (parse_array_dim_opt given)
  ) test_cases
