...
let test_optimize_expr_simple () : unit =
  ...
  Alcotest.check Alcotest.(result int string) "maximize constant 42" (Ok 42) result
...
let test_optimize_expr_minimize () : unit =
  ...
  Alcotest.check Alcotest.(result int string) "minimize constant 10" (Ok 10) result
...
let test_optimize_expr_with_variable () : unit =
  ...
  Alcotest.check Alcotest.(result int string) "maximize x where x <= 5" (Ok 5) result
...
let test_optimize_expr_arithmetic () : unit =
  ...
  Alcotest.check Alcotest.(result int string) "maximize x + 3 where x <= 2" (Ok 5) result
...
let test_optimize_expr_minimize_with_constraint () : unit =
  ...
  Alcotest.check Alcotest.(result int string) "minimize x where x >= 10" (Ok 10) result
...
let test_optimize_expr_unsat () : unit =
  ...
  in
  let result = IntGen.optimize_expr ~pre Optimizer.Strategy.Maximize (Var x) in
  Alcotest.check Alcotest.(result int string) "contradictory constraints should be unsat" (Error "unsat") result
...
let test_optimize_expr_multiplication () : unit =
  ...
  let result = IntGen.optimize_expr ~pre Optimizer.Strategy.Maximize expr in
  Alcotest.check Alcotest.(result int string) "maximize x * 2 where x <= 3" (Ok 6) result

let test_optimize_expr_with_timeout () : unit =
  (* Test with timeout parameter *)
  let result = IntGen.optimize_expr ~timeout:1000 Optimizer.Strategy.Maximize (Num 7) in
  Alcotest.check Alcotest.(result int string) "maximize with timeout" (Ok 7) result
...
let tests : unit Alcotest.test_case list =
  [
    ...
    ("optimize_expr_minimize_with_constraint", `Quick, test_optimize_expr_minimize_with_constraint);
