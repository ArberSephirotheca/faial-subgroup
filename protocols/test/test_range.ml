...
let inc_range (lb : int) (ub : int) (inc : int -> int) : int list =
  let rec iter n = if n <= ub then n :: iter (inc n) else [] in
  iter lb

let dec_range (lb : int) (ub : int) (dec : int -> int) : int list =
  let rec iter n = if n >= lb then n :: iter (dec n) else [] in
  iter ub
...
module ForStep = struct
  type kind = Plus | Minus | Div | Mult
  type t = kind * int

  let to_range : t -> Range.Step.t * Range.direction = function
...
let test_all () : unit =
  ...
  List.concat_map
    (fun lb ->
      List.concat_map
        (fun ub -> List.map (fun s -> (lb, ub, s)) [ 1; 2; 3; 4; 5 ])
...
let tests : unit Alcotest.test_case list =
  [
    ("plus_range", `Quick, test_plus_range);
    ("minus_range", `Quick, test_minus_range);
    ("mult_range", `Quick, test_mult_range);
    ("div_range", `Quick, test_div_range);
    ("first", `Quick, test_first);
    ("last", `Quick, test_last);
    ("gen_highest_power", `Quick, test_gen_highest_power);
    ("highest_power", `Quick, test_highest_power);
    ("all", `Quick, test_all);
  ]

let () = Alcotest.run "Range" [ ("tests", tests) ]
