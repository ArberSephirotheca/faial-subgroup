...
let list_theorems () =
  ...
  List.iteri (fun i (title, thm_fn) ->
    let cfg = make_config 32 (Dim3.make ~x:32 ()) in (* Use default config for display *)
...
let run_benchmarks ~(strategy : Constraints.t) ~(threads_per_warp : int) ~(all : bool) ~(theorem : int) ~(mode : BenchmarkMode.t) ~(block_dim : Dim3.t) =
  let strategies =
...
let main (strategy:Constraints.t) (threads_per_warp:int) (theorem:int) (all:bool) (list_thms:bool) (mode:BenchmarkMode.t) (block_dim:Dim3.t) : unit =
  if list_thms then
    list_theorems ()
  else
    run_benchmarks ~strategy ~all ~threads_per_warp ~theorem ~mode ~block_dim
...
let benchmark_mode_conv : BenchmarkMode.t Arg.conv =
  let parse s =
    ...
    | Some v -> Ok v
    | None ->
        Error (`Msg (Printf.sprintf "Invalid mode '%s'. Valid options are: prove, p, optimize, o" s))
  in
...
let mode_arg =
  ...
  Arg.(value & opt benchmark_mode_conv BenchmarkMode.default & info ["m"; "mode"] ~doc)
...
let main_cmd =
  ...
  Cmd.v info Term.(const main $ strategy_arg $ threads_arg $ theorem_arg $ all_arg $ list_theorems_arg $ mode_arg $ block_dim_arg)
