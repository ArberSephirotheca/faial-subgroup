...
module Solver = struct
  ...
  let get_cost (app : t) (r1 : Ra.Stmt.t) (r2 : Ra.Stmt.t) :
      (cost, string) Result.t =
    if app.show_ra then (
    ...
    (if app.use_maxima then
       ...
       |> Maxima.run_exe ~verbose:app.show_code ~exe:app.maxima_exe
     else
  ...
  type summary = TotalCost of ((kernel * kernel) * r_cost) list
  ...
  let run (s : t) : summary =
    ...
    let kernels2 =
      s.kernels2 |> clean |> List.map (fun k -> (Kernel.name k, k))
    in
    let ks =
      List.filter_map
        (fun k1 ->
...
module TUI = struct
  let run ~only_cost (s : Solver.t) =
    ...
    | TotalCost [] -> abort_when (not s.ignore_absent) "No kernels found."
    | TotalCost l -> List.iter print_r_cost l
end
...
module JUI = struct
  ...
  let to_json (s : Solver.t) : json =
    let c_to_j name (((k1, _) : kernel * kernel), r) : json =
      ...
      | Error e ->
          `Assoc [ ("kernel_name", `String k1.name); ("error", `String e) ]
    in
    let kernels =
      match Solver.run s with
...
let pico (fname1 : string) (fname2 : string) (block_dim : Dim3.t option)
    (grid_dim : Dim3.t option) (use_maxima : bool) (show_ra : bool)
    (show_code : bool) (maxima_exe : string) (skip_simpl_ra : bool)
    (only_cost : bool) (ignore_absent : bool) (output_json : bool)
  ...
  let parse fname =
    ...
    parsed.kernels
  in
  let block_dim : Dim3.t =
    Option.value ~default:(Dim3.make ~x:32 ()) block_dim
  in
