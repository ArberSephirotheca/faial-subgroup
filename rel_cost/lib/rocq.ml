...
let from_kernel ?(syntax = Syntax.constructor) (k : Kernel.t) :
    (Indent.t list, error) Result.t =
  let ( let* ) = Result.bind in
  let cfg = make_config ~syntax ~metric:"(MemReads.T m)" () in
...
let from_kernels ?(syntax = Syntax.constructor) (ks : Kernel.t list) :
    (Indent.t list, error) Result.t =
  let ( let* ) = Result.bind in
  let* mods =
    List.fold_left
      (fun acc k ->
        let* acc = acc in
        let* m = from_kernel ~syntax k in
        Ok (acc @ m @ [ Indent.Line "" ]))
      (Ok []) ks
  in
  let header_comment =
    Printf.sprintf "(* Generated by faial-to-rocq (%s backend). *)" syntax.name
  in
  let header_lines =
