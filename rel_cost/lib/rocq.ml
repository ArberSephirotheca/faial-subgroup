...
(** [include_imports = false] suppresses both the common
    ...
    generating-tool comment is still emitted as a one-liner so the
    output's provenance survives a copy-paste into another file. *)
let from_kernels ?(syntax = Syntax.constructor) ?(metric = Metric.MemReads)
    ?(include_imports = true) (ks : Kernel.t list) :
    (Indent.t list, error) Result.t =
  let ( let* ) = Result.bind in
  let* mods =
    List.fold_left
  ...
  in
  Ok ((Indent.Line header_comment :: preamble) @ mods)
