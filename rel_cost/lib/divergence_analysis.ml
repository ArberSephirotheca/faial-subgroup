...
let rec c_is_uniform (locals : Variable.Set.t) : Bank.Code.t -> bool = function
  | Index _ -> true
  | Loop { range = r; body = p } ->
...
let n_is_divergent : Exp.nexp -> bool = Exp.n_exists Variable.is_tid
let r_is_divergent : Range.t -> bool = Range.exists Variable.is_tid
let b_is_divergent : Exp.bexp -> bool = Exp.b_exists Variable.is_tid

let rec c_is_divergent : Bank.Code.t -> bool = function
  | Index _ -> false
...
type t = ThreadUniform | ThreadDivergent | PossiblyThreadDivergent

let is_thread_uniform (x : t) : bool = x = ThreadUniform

let is_known : t -> bool = function
  | ThreadUniform | ThreadDivergent -> true
  | PossiblyThreadDivergent -> false

let from_bank (b : Bank.t) : t =
  if is_uniform b then ThreadUniform
  else if is_divergent b then ThreadDivergent
  else PossiblyThreadDivergent
